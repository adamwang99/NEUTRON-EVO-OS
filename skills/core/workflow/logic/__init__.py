...
def _write_pending_learned(candidates: list[dict], task: str) -> None:
    """
    Write distill_patterns() candidates to memory/pending/LEARNED_pending.md.
    This closes the distill → action loop: candidates get human review
    before becoming permanent LEARNED.md entries.
    """
    from engine._atomic import atomic_write

    pending_path = MEMORY_DIR / "pending" / "LEARNED_pending.md"
    ...
    for c in candidates:
        entries.append(
            f"## [{timestamp}] Distill Pattern\n"
            f"- **Pattern:** {c.get('pattern', 'unknown')[:120]}\n"
            f"- **Frequency:** {c.get('frequency', 0)}x across recent sessions\n"
            f"- **Last seen:** {c.get('last_seen', '-')}\n"
            f"- **Task context:** {task[:100]}\n"
            f"- **Status:** pending_review\n\n"
        )

    header = f"<!-- DISTILL CANDIDATES — auto-generated by workflow/{timestamp} -->\n\n"
    try:
        existing = ""
        if pending_path.exists():
            existing = pending_path.read_text()
        # Keep only the last 50KB to prevent unbounded growth
        if len(existing) > 50_000:
            existing = existing[-40_000:]
        atomic_write(pending_path, existing + header + "\n".join(entries) + "\n")
    except Exception:
        pass  # best-effort: never block delivery on pending write failure
...
def _step_ship(task: str, context: dict) -> dict:
    ...
    # ── Self-evolution: distill patterns from session into LEARNED.md pending ─────
    # distill_patterns() output was previously DISCARDED — now written to pending/.
    # This closes the distill → action loop: patterns become pending reviews.
        ...
        try:
            ...
            def _distill():
                try:
                    ...
                    candidates = distill_patterns(min_frequency=3, top_n=3)
                    if candidates:
                        _write_pending_learned(candidates, task)
                except Exception as _e:
                    print(f"[WORKFLOW] distill failed: {_e}", file=_sys.stderr)
