...
1. This file was auto-generated by NEUTRON EVO OS extension
2. AI assistants will automatically read this file
3. For more info, see [NEUTRON EVO OS](https://github.com/adamwang99/NEUTRON-EVO-OS)

---

*Powered by NEUTRON EVO OS — Functional Credibility Over Institutional Inertia*
`,
...
export function activate(extContext: vscode.ExtensionContext) {
	...
	console.log('[NEUTRON EVO OS] Extension activated!');

	// Register commands
	const applyCmd = vscode.commands.registerCommand('neutron-evo-os.apply', applyToWorkspace);
	const setupCmd = vscode.commands.registerCommand('neutron-evo-os.setup', quickSetup);
	const openDocsCmd = vscode.commands.registerCommand('neutron-evo-os.openDocs', openDocumentation);
...
async function autoInjectIfEnabled() {
	const config = vscode.workspace.getConfiguration('neutronEvoOs');
...
async function injectTemplate(workspacePath: string): Promise<boolean> {
	const config = vscode.workspace.getConfiguration('neutronEvoOs');
	const files = config.get<string[]>('files', ['CLAUDE.md', 'SOUL.md', 'USER.md', 'GOVERNANCE.md', 'RULES.md', 'MANIFESTO.md', 'PERFORMANCE_LEDGER.md']);
	...
	for (const pattern of excludePatterns) {
		if (matchGlob(workspacePath, pattern)) {
			console.log(`[NEUTRON EVO OS] Skipping excluded folder: ${workspacePath}`);
			return false;
		}
	}
	...
	if (!fs.existsSync(claudeSettingsPath)) {
		...
		console.log(`[NEUTRON EVO OS] Created: ${claudeSettingsPath}`);
		createdCount++;
	}
...
async function applyToWorkspace() {
	...
	const files = ['CLAUDE.md', 'SOUL.md', 'USER.md', 'GOVERNANCE.md', 'RULES.md', 'MANIFESTO.md', 'PERFORMANCE_LEDGER.md'];
...
async function injectTemplateWithFiles(workspacePath: string, files: string[]): Promise<boolean> {
	...
	if (createdCount > 0) {
		...
		vscode.window.showInformationMessage('NEUTRON EVO OS: Context files already exist.');
	}
	return true;
}
...
async function quickSetup() {
	...
	switch (choice.label) {
		...
		case '✅ Disable Auto-Inject':
			...
			await vscode.workspace.getConfiguration('neutronEvoOs').update('neutronEvoOs.autoInject', newAutoInject);
		...
		case 'Configure Settings':
			vscode.commands.executeCommand('workbench.action.openSettings', 'neutronEvoOs');
			break;
	}
}
