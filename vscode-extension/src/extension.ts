...
async function autoInjectCurrentWorkspace() {
	...
	const autoInject = getConfig<boolean>('autoInject', true);
	if (!enabled || !autoInject) return;
...
async function onWorkspaceFoldersChanged(event: vscode.WorkspaceFoldersChangeEvent) {
	const enabled = getConfig<boolean>('enabled', true);
	const autoInject = getConfig<boolean>('autoInject', true);
	if (!enabled || !autoInject) return;
...
async function injectToWorkspace(wsPath: string): Promise<boolean> {
	...
	const filesToInject = getConfig<string[]>('files', DEFAULT_FILES);
	const templateSource = getConfig<string>('templateSource', 'embedded');
	...
	for (const file of filesToInject) {
		...
		if (!fs.existsSync(filePath)) {
			...
			if (templateSource === 'external') {
				const customPath = getConfig<string>('customTemplatePath', '');
			...
			if (content) {
				...
				content = content.replace(/\\u([0-9a-f]{4})/gi, (_, code) =>
	...
	if (createdCount > 0) {
		vscode.window.showInformationMessage(
			`NEUTRON EVO OS: Created ${createdCount} context file(s)!`
		);
	}
...
async function ensureClaudeSettings(wsPath: string): Promise<void> {
	...
	switch (choice.label) {
		...
		case 'Open Documentation':
			vscode.commands.executeCommand('vscode.open',
				vscode.Uri.parse('https://github.com/adamwang99/NEUTRON-EVO-OS'));
			break;
...
async function openDocs() {
	...
	if (choice?.label === 'GitHub Repository') {
		...
		vscode.window.showInformationMessage(
			'Full context files available after installing NEUTRON EVO OS globally. Run: Install Globally'
		);
	}
}
...
function getConfig<T>(key: string, defaultValue: T): T {
	return vscode.workspace.getConfiguration(CONFIG_PREFIX).get<T>(key, defaultValue) ?? defaultValue;
}
...
function showWelcome() {
	...
	const shown = config.get<boolean>(shownKey, false);

	if (!shown) {
		vscode.window.showInformationMessage(
			'NEUTRON EVO OS v4.1.0 activated! Run "NEUTRON EVO OS: Quick Setup" to configure.',
